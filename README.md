...
1. 启动工具，点击“安装 CS2 监听配置…”。程序会先从正在运行的 CS2 进程、Steam 注册表和 Steam 游戏库配置中自动查找 `cs2.exe`。
2. 确认自动检测到的位置即可安装；如果没有检测到或需要使用其他位置，仍可手动选择 CS2 安装目录内的 `game\bin\win64\cs2.exe`。
...
本程序只使用 Valve 公开提供的 Game State Integration（GSI）接收本机游戏状态，不注入或修改 CS2，不读取游戏内存，不自动瞄准、移动或射击，也不绕过 VAC。从当前实现范围看，它不具备外挂功能。

不过，本程序并非 Valve、Steam、完美世界或任何赛事平台的官方软件，也未获得官方合规认证。平台规则和判定方式可能变化，因此不能承诺在所有平台、赛事或未来版本中一定被允许，使用前请自行确认所在平台和赛事规则。

当前版本仍在测试。死亡检测、切换窗口和下一回合切回可能受网络、全屏模式、系统权限或游戏更新影响而失败或延迟。**不建议正在排位、冲分或重视比赛结果的玩家使用**，建议先在休闲模式或练习环境中测试。
